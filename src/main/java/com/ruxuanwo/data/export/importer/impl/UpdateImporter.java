...
/**
 * 多关联更新导入器
 * 可以导入一对一、一对多、多对多关系型数据，并支持重复数据进行更新操作
 * 原理：插入数据前进行查询是否有该数据
 * @author ruxuanwo
 */
@Component
public class UpdateImporter implements Importer {
            ...
            for (Map<String, Object> dataMap : dataList) {
                for (EdTemplateTableDTO table : list) {
                    ...
                    if (keyBack == null) {
                        ...
                        table.putKeyAndValue(table.getKeyName(), keyBack.toString());
                        table.createUpdatePrepareSql();
                        DataBaseUtil.updatePrepare(connection, table.getUpdatePrepareSql().toString(), table.getKeyAndValue(), keyBack);
                    }
                }
            }
        } catch (Exception e) {
